...
const PLUGIN_ID = 'edu.hot.rwa-edu'
...
async function runSimulate() {
  ...
  try {
    const res = await fetch(`/api/v1/labs/${PLUGIN_ID}/simulate`, {
      ...
      body: JSON.stringify({
        user_prompt: '虚构资产链上映射教学（非募资/发行）',
        params: { asset_id: param.value },
...
<template>
  <section class="lab-panel">
    <header class="lab-header">
      ...
      <div>
        <h1>RWA 链上映射教学</h1>
        <p class="muted">edu.hot.rwa-edu · 测试网教学 only</p>
      </div>
    </header>
    <div class="card">
      <label>虚构资产 ID
