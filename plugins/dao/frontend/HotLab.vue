...
<template>
  <section class="lab-panel">
    ...
    <div class="lab-grid">
      <div class="card">
        <h2>{{ t('proposalSection') }}</h2>
        <label>{{ t('proposalId') }} <input v-model="proposalId" /></label>
        <label>{{ t('titleLabel') }} <input v-model="proposalTitle" /></label>
        <label>{{ t('quorumLabel') }} <input v-model.number="quorum" type="number" min="1" /></label>
        <label>{{ t('voterLabel') }} <input v-model="voterName" /></label>
        <div class="vote-actions">
          <button type="button" class="yes" @click="castVote(true)">{{ t('voteYes') }}</button>
          <button type="button" class="no" @click="castVote(false)">{{ t('voteNo') }}</button>
        </div>
        <button class="submit" :disabled="loading" @click="submit">
          {{ loading ? t('onChainSim') : t('submitVote') }}
        </button>
      ...
      <div class="card">
        ...
        <div class="counts">
          <span class="yes">{{ t('yes') }} {{ displayYes }}</span>
          <span class="no">{{ t('no') }} {{ displayNo }}</span>
        </div>
        <p class="muted">{{ t('turnout', { pct: turnout, target: displayQuorum }) }}</p>
        <p class="muted">{{ t('localCount', { yes: yesVotes, no: noVotes }) }}</p>
      </div>

      <div class="card">
        <h2>{{ t('govFlow') }}</h2>
        <ol class="gov-flow">
          <li :class="{ done: proposalTitle.length > 0 }">{{ t('flow_draft') }}</li>
          <li :class="{ done: totalVotes > 0 }">{{ t('flow_vote') }}</li>
          <li :class="{ done: status === 'passed' || status === 'rejected' }">{{ t('flow_tally') }}</li>
          <li :class="{ done: taskStatus === 'completed' }">{{ t('flow_record') }}</li>
        </ol>
        <p class="muted">{{ t('contractTemplate') }}: <code>DaoVoteDemo.sol</code></p>
      </div>
    </div>
